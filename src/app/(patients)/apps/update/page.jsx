...
export default function UpdatePatientPage() {
    ...
    const { updatePatient, patients } = usePatientsContext();
            ...
            try {
                ...
                setPatientData({
                    ...
                    birthDate: foundPatient.birthDate
                        ? formatDate(foundPatient.birthDate, "yyyy-MM-dd")
                        : "",
                ...
                setHasCompanion(
                    !!(foundPatient.companionName || foundPatient.companionCpf)
                );
            } catch (error) {
                toast.error(error.message || "Erro ao buscar paciente.");
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await updatePatient(patientId, updatedPatient);

            if (response.status !== 200 || !response.data) {
                toast.error(
                    `${response.status}: Erro ao atualizar paciente. Verifique os dados e tente novamente.`
                );
                return;
            }
